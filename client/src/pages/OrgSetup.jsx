...
const input = 'mt-1 w-full rounded-md border border-[#e0dedb] px-3 py-2 text-sm focus:border-[#37322f] focus:outline-none focus:ring-1 focus:ring-[#37322f]'

export default function OrgSetup() {
  const [tab, setTab] = useState('Departments')
  return (
    <div>
      <h1 className="text-2xl font-bold text-[#37322f]">Organization Setup</h1>
      <p className="mt-1 text-sm text-[#847d76]">Master data — admin only.</p>
      <div className="mt-5 flex gap-1 border-b border-[#e6e3df]">
        {TABS.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`-mb-px border-b-2 px-4 py-2 text-sm font-medium ${
              tab === t ? 'border-[#37322f] text-[#4b453f]' : 'border-transparent text-[#847d76] hover:text-[#4b453f]'
            }`}
          >
            {t}
          </button>
    ...
    { key: 'status', header: 'Status', render: (r) => <StatusPill status={r.status} /> },
    {
      key: 'actions', header: '', render: (r) => (
        <div className="flex gap-2">
          <button onClick={() => startEdit(r)} className="text-[#37322f] hover:underline">Edit</button>
  ...
  return (
    <div>
      <div className="mb-3 flex justify-end">
        <button onClick={startAdd} className={`${btn} bg-[#37322f] text-white hover:bg-[#4b453f]`}>+ Add Department</button>
      </div>
      ...
      {open && (
        <Modal
          ...
          footer={
            <>
              <button onClick={() => setOpen(false)} className={`${btn} border border-[#e0dedb] text-[#605a57]`}>Cancel</button>
              <button onClick={save} className={`${btn} bg-[#37322f] text-white`}>Save</button>
            </>
          }
        >
          ...
          <label className="mt-3 block text-sm font-medium text-[#605a57]">Head</label>
          ...
          <label className="mt-3 block text-sm font-medium text-[#605a57]">Parent department</label>
          ...
          <label className="mt-3 block text-sm font-medium text-[#605a57]">Status</label>
...
function CategoriesPanel() {
  ...
  const columns = [
    ...
    { key: 'assets', header: 'Assets', render: (r) => r._count?.assets ?? 0 },
    { key: 'status', header: 'Status', render: (r) => <StatusPill status={r.status} /> },
    { key: 'actions', header: '', render: (r) => <button onClick={() => startEdit(r)} className="text-[#37322f] hover:underline">Edit</button> },
  ]

  return (
    <div>
      <div className="mb-3 flex justify-end">
        <button onClick={startAdd} className={`${btn} bg-[#37322f] text-white hover:bg-[#4b453f]`}>+ Add Category</button>
      </div>
      ...
      {open && (
        ...
        >
          ...
          <label className="text-sm font-medium text-[#605a57]">Name</label>
          <input className={input} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
          <label className="mt-3 block text-sm font-medium text-[#605a57]">Warranty period (months, optional)</label>
...
function EmployeesPanel() {
  ...
  const columns = [
    ...
    { key: 'role', header: 'Role', render: (r) => <span className="rounded bg-[#f0ebe4] px-2 py-0.5 text-xs font-medium text-[#4b453f]">{r.role}</span> },
    { key: 'status', header: 'Status', render: (r) => <StatusPill status={r.status} /> },
    {
      key: 'actions', header: 'Set role', render: (r) => (
        <select
          ...
          className="rounded-md border border-[#e0dedb] px-2 py-1 text-xs focus:border-[#37322f] focus:outline-none"
        >
  ...
  return (
    <div>
      <p className="mb-3 text-sm text-[#847d76]">Promote a member by changing their role. Enforced server-side (Admin only).</p>
...
function StatusPill({ status }) {
  ...
  return (
    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${active ? 'bg-green-50 text-green-700' : 'bg-[#efece8] text-[#847d76]'}`}>
      {status}
    </span>
  )
}
